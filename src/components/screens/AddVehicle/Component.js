...
class AddVehicle extends Component {
    ...
    if (text.length >= 2) {
      ...
      fetch(
        'https://maps.googleapis.com/maps/api/place/autocomplete/json?&input=' +
          text +
          '&location=-34.605914,-58.453213&radius=200000&strictbounds&key=' +
      ...
      try {
        ...
        if (url) {
          ...
          if (
            ...
            result.data.editUser.profile &&
            result.data.editUser.profile.vehical
          ) {
          ...
          this.setState({
            disabledButton: false,
            errorText: 'No se puede cargar avtar',
          });
        }
      } catch (error) {
        this.setState({
          disabledButton: false,
          errorText: 'No se puede agregar vehículo',
        });
      }
    }
  };
    ...
    return (
      <TextInput
        ...
        onChangeText={changedText =>
          this.handleInput({key, value: changedText})
        }
  ...
  renderModal = () => {
    return (
      <ModalBox
        ...
        <VehicleTypes
          handleVehicleType={vehicleType => this.handleVehicleType(vehicleType)}
          closeModal={() => this.setState({displayModal: false})}
        />
      </ModalBox>
    );
  ...
  setImage = response => {
    ...
    media['extension'] = getFileExtension(
      response.fileName,
    ).toLocaleLowerCase();
    this.setState({media});
  };
      ...
      <View style={styles.menuWrapper}>
        {this.state.neighborhoods.map((neighborhood, index) => {
          return (
            <TouchableOpacity
              activeOpacity={0.8}
              onPress={() => this.handleNeighborhoodSelection(neighborhood)}
              key={index}>
        ...
        <KeyboardAwareScrollView
          ...
          {errorText && (
            <WrongInputWarning
              ...
              style={styles.wrongInputWarningWrapper}
            />
          )}
          <TextInput
...
const withMutation = graphql(EDIT_USER_MUTATION, {name: 'editUser'})(
  AddVehicle,
);
